...
def load_itr_json(itr_path: str) -> Dict:
    ...
    # Salary income
    income_data['salary_income'] = income_section.get('GrossSalary', 0.0) or income_section.get('Salary', 0.0) or 0.0
    income_data['net_salary'] = income_section.get('NetSalary', income_data['salary_income'])
    income_data['income_from_salary'] = income_section.get('IncomeFromSal', 0.0)
    
    # Business income (for ITR-4)
    if itr_form == 'ITR-4':
        business_section = form_data.get('ScheduleBP', {})
        presumptive_inc = business_section.get('PersumptiveInc44ADA', {})
        income_data['business_income'] = presumptive_inc.get('TotPersumptiveInc44ADA', 0.0) or \
                                         business_section.get('PersumptiveInc44AE', {}).get('TotPersumInc44AE', 0.0) or \
                                         income_section.get('IncomeFromBusinessProf', 0.0) or 0.0
    else:
        income_data['business_income'] = 0.0
    
    # House property
    income_data['house_property_income'] = income_section.get('TotalIncomeOfHP', 0.0)
    
    # Capital gains
    if 'LTCG112A' in form_data:
        ltcg = form_data['LTCG112A']
        income_data['capital_gains'] = ltcg.get('LongCap112A', 0.0)
    else:
    ...
    other_income_total = sum(item.get('OthSrcOthAmount', 0.0) for item in other_income_list)
    income_data['other_income'] = income_section.get('IncomeOthSrc', other_income_total)
    
    # Total income
    income_data['gross_total_income'] = income_section.get('GrossTotIncome', 0.0)
    income_data['net_taxable_income'] = income_section.get('TotalIncome', 0.0)
    
    # Extract deductions
    deductions_data = {}
    deductions_section = income_section.get('DeductUndChapVIA', {})
    deductions_data['section_80c'] = deductions_section.get('Section80C', 0.0)
    deductions_data['section_80d'] = deductions_section.get('Section80D', 0.0)
    deductions_data['section_24b'] = 0.0  # Home loan interest (not in standard structure)
    deductions_data['total_deductions'] = deductions_section.get('TotalChapVIADeductions', 0.0)
    
    # Extract tax data
    tax_data = {}
    taxes_paid = tax_paid_section.get('TaxesPaid', {})
    tax_data['tax_paid'] = taxes_paid.get('TotalTaxesPaid', 0.0)
    tax_data['tds_deducted'] = taxes_paid.get('TDS', 0.0)
    tax_data['tax_refund'] = form_data.get('Refund', {}).get('RefundDue', 0.0)
    
    # Tax outstanding
    tax_data['tax_outstanding'] = tax_paid_section.get('BalTaxPayable', 0.0)
    
    # Tax computation
    tax_liability = tax_section.get('NetTaxLiability', 0.0)
    if tax_liability == 0:
        tax_liability = tax_section.get('TotalTaxPayable', 0.0)
